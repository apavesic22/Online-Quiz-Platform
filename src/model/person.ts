...
export class Person {
  ...
  constructor(firstname: string, lastname: string, birthdate: Date, email: string) {
    if (!firstname || typeof firstname !== 'string' || (firstname = firstname.trim()).length === 0)
      throw new HttpError(400, 'First name was not provided correctly');
    if( !lastname || typeof lastname !== 'string' || (lastname = lastname.trim()).length === 0)
    ...
    if( !email || typeof email !== 'string' || (email = email.trim()).length === 0 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
