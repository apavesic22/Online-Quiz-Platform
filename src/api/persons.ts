...
/**
 * @api {get} /api/persons Retrieving persons
 * @apiGroup Persons
 *
 * @apiQuery {String} [q] Filtering pattern (SQL style)
 * @apiQuery {Number} [sort=0] Column number to sort (negative - descending sort)
 * @apiQuery {Number} [offset=0] Record number to start retrieving
 * @apiQuery {Number} [limit=10] Limit of the records
 *
 * @apiSuccess {Number} total Total number of persons
 * @apiSuccess {Number} filtered Number of persons after filtering
 * @apiSuccess {Object[]} persons List of person objects
 * @apiSuccess {Number} persons.id Person ID
 * @apiSuccess {String} persons.firstname First name
 * @apiSuccess {String} persons.lastname Last name
 * @apiSuccess {Date} persons.birthdate Birthdate
 * @apiSuccess {String} persons.email Email
 * @apiSuccess {Object[]} persons.team_objects List of team objects which the person belongs to
 * @apiSuccess {Number} persons.team_objects.id Team ID
 * @apiSuccess {String} persons.team_objects.name Team name
 * @apiSuccess {String} persons.team_objects.longname Team long name
 * @apiSuccess {String} persons.team_objects.color Team color (for background if no avatar)
 * @apiSuccess {Number} persons.team_objects.has_avatar Team avatar uploaded (1 if yes)
 * @apiUse HttpError
*/
personsRouter.get('/', requireRole([0, 1]), async (req: Request, res: Response) => {
  ...
  if (q) { // filter query provided
    let concat = Object.entries(personTableDef.columns)
      .filter(([_name, def]) => !('skipFiltering' in def))
      .map(([name, def]) => {
      if (def.type === 'DATE') {
  ...
  if (!isNaN(offset) && offset > 0) { // offset provided
    query += ' OFFSET ?';
...
/**
 * @api {post} /api/persons Adding new person
 * @apiGroup Persons
 *
 * @apiBody {String} firstname First name
 * @apiBody {String} firstname Last name
 * @apiBody {String} birthdate Birth date in any format which can be used as a parameter of Date() constructor
 * @apiBody {String} email Email
 * @apiBody {Number[]} team_ids Array of teams ids the person belongs to
 *
 * @apiSuccess {Number} id Id of the just added person (automatically generated)
 * @apiSuccess {String} firstname First name
 * @apiSuccess {String} firstname Last name
 * @apiSuccess {String} birthdate Birth date
...
/**
 * @api {put} /api/persons Modifying existing person
 * @apiGroup Persons
 *
 * @apiBody {Number} id Id of the person to modify
 * @apiBody {String} firstname First name
 * @apiBody {String} firstname Last name
 * @apiBody {String} birthdate Birth date in any format which can be used as a parameter of Date() constructor
 * @apiBody {String} email Email
 * @apiBody {Number[]} team_ids Array of teams ids the person belongs to
 *
 * @apiSuccess {Number} id Id of the modified person
 * @apiSuccess {String} firstname First name
 * @apiSuccess {String} firstname Last name
 * @apiSuccess {String} birthdate Birth date
 * @apiSuccess {String} email Email
...
/**
 * @api {delete} /api/persons Deleting existing person
 * @apiGroup Persons
 *
 * @apiQuery {Number} id Id of the person to delete
 *
 * @apiSuccess {Number} id Id of the just deleted person
